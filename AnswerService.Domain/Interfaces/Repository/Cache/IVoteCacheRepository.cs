...
public interface IVoteCacheRepository
{
    ...
    /// <summary>
    ///     Retrieves votes grouped by answer identifiers from the cache.
    ///     If any answer or votes are missing, the data is fetched using <paramref name="fetch" /> and cached accordingly.
    /// </summary>
    /// <param name="answerIds">The list of answer IDs whose votes should be retrieved.</param>
    /// <param name="fetch">A fallback delegate that fetches missing votes grouped by answer ids.</param>
    /// <param name="cancellationToken">A cancellation token for the asynchronous operation.</param>
    /// ///
    /// <returns>
    ///     A <see cref="IEnumerable{Vote}" /> containing a lookup-like list of answer Ids to the list of votes.
    /// </returns>
    Task<IEnumerable<KeyValuePair<long, IEnumerable<Vote>>>> GetAnswersVotesAsync(IEnumerable<long> answerIds,
    ...
    ///     If any vote type or associated votes are missing, the data is fetched using <paramref name="fetch" /> and cached
    ///     accordingly.
    /// </summary>
